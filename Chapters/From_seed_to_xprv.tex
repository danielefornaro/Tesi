...
In this chapter we will see how an HD wallet [\cite{1}] works.
...
An Extended Key is a sequence of bytes, encoded in base 58. It contains all the information necessary derive the keys. When the derivation is made for the first time from the seed, the extended key is called master key.\\ \\
...
\end{equation*}
where $msg$ is a string of $37$ bytes. \\ \\
Finally we apply the HMAC algorithm with the following input:
...
\end{equation*}
where $msg$ is a string of $37$ bytes. \\ \\
Then we apply the HMAC algorithm with the following input:
...
\begin{itemize}
	...
	\item Index.
\end{itemize}
First we apply the HMAC algorithm to the same input used for the normal derivation:
\\ \\
Then we concatenate the 33 byte string of the public key to the 4 byte string representing the index number:

\begin{equation*}
msg = compressed \; public\;key \;|\; index
\end{equation*}
After that we apply the HMAC algorithm with the following input:
...
Now we multiply the generator $G$ to the integer number $q$ and obtain $Q$, a point on the EC:
\begin{equation*}
Q=q\cdot G.
\end{equation*}
Finally we compute the sum between two point on the elliptic curve: $Q$ and $P_{parent}$, where $P_{parent}$ is the parent public key.
\begin{equation*}
...
\end{equation*}
Then we apply the HMAC algorithm with the usual input:
